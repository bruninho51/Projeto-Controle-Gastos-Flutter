...
class NotificacaoEdicaoPage extends StatefulWidget {
  final NotificacaoBancariaModel notificacao;
  final String apiToken;

  const NotificacaoEdicaoPage({
    super.key,
  ...
  @override
  State<NotificacaoEdicaoPage> createState() => _NotificacaoEdicaoPageState();
}

class _NotificacaoEdicaoPageState extends State<NotificacaoEdicaoPage> {
  ...
  Future<void> _carregarDescricaoSalva() async {
    try {
      final mapeamento = await NotificacoesChannel.buscarMapeamento(
        widget.notificacao.descricaoOriginal,
      );
      if (mapeamento != null &&
          mapeamento.descricaoNormalizada.isNotEmpty &&
          mapeamento.descricaoNormalizada != widget.notificacao.descricaoOriginal &&
          mounted) {
        setState(() {
  ...
  Future<void> _salvar() async {
    ...
    try {
      await NotificacoesChannel.update(
        id: widget.notificacao.id,
        valor: _parsedValor(),
        descricaoNormalizada: desc,
      );
      await NotificacoesChannel.salvarMapeamento(
  ...
  Future<void> _apagar() async {
    ...
    try {
      await NotificacoesChannel.delete(widget.notificacao.id);
      if (mounted) {
        OrcamentosSnackBar.success(
    ...
    showModalBottomSheet(
      ...
      builder: (_) => _CadastrarGastoSheet(
        ...
        onSucesso: (gastoId) async {
          await NotificacoesChannel.associarGasto(
            id: widget.notificacao.id,
            gastoId: gastoId,
          );
          ...
          Expanded(
            child: SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  // ── Cartão de informações originais ──────────────────────
                  _SectionCard(
                    children: [
                      _InfoRow(
                        label: 'Banco',
