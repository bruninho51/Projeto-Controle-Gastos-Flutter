...
class _DetalhesGastoFixoPageState extends State<DetalhesGastoFixoPage> {
  ...
  final _updateValorPrevistoFormKey = GlobalKey<FormState>();
  ...
  Future<void> deleteGastoFixo(int gastoFixoId) async {
    ...
    if (response.statusCode == 200) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Gasto fixo apagado com sucesso!')),
      );
      Navigator.pop(context, true); // Retorna à tela anterior
    } else {
      throw Exception('Falha ao apagar o gasto fixo');
    }
  }
...
final client = await MyHttpClient.create();
    ...
    if (response.statusCode >= 200 && response.statusCode <= 299) {
      ...
      setState(() {
        print('orcamenot ${result}');
        orcamento = result; // Atualiza o 'gasto' com os dados recebidos
      });
    } else {
...
final client = await MyHttpClient.create();
  ...
  // Função para enviar a atualização do gasto via PATCH
  Future<void> _updateGasto(int gastoId, String valor, String dataPgto) async {
    final orcamentoId = widget.orcamentoId;
...
final client = await MyHttpClient.create();
  ...
  void _openValorPrevistoDialog(BuildContext context) {
    showDialog(
      context: context,
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: Form(
            key: _updateValorPrevistoFormKey,
            child: Column(
              mainAxisSize: MainAxisSize.min,
              children: [
                // Campo de valor
          ...
          actions: <Widget>[
            TextButton(
              onPressed: () {
                String valor = converterParaFormatoNumerico(_valorController.text);
                  if (_updateValorPrevistoFormKey.currentState?.validate() ?? false) {
                    _updateValorPrevisto(gasto['id'], valor);
                    _getGasto(widget.gastoId);
  ...
  void _openPaymentDialog(BuildContext context) {
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          title: const Text('Pagamento de Gasto'),
          content: Form(
            key: _updatePaymentFormKey,
            child: Column(
              mainAxisSize: MainAxisSize.min,
              children: [
                // Campo de valor
          ...
          actions: <Widget>[
            TextButton(
              onPressed: () {
                ...
                if (_updatePaymentFormKey.currentState?.validate() ?? false) {
                  _updateGasto(gasto['id'], valor, dataPgto);
                  _getGasto(widget.gastoId);
                  _dataController.text = '';
                  _valorController.text = '';
                  Navigator.pop(context, true);
                ...
                child: Column(
                  ...
                  children: [
                      ...
                      children: [
                        Expanded(
                          child: _buildDetailCard(
                            ...
                            value: formatarValorDynamic(gasto['previsto']),
                            color: Colors.green,
                            icon: Icons.attach_money,
                            onTap: orcamento['data_encerramento'] == null ? () {
                              _openValorPrevistoDialog(context); 
                            }: null
                          ),
                        ),
                      ...
                      Row(
                        ...
                        children: [
                          Expanded(
                            child: _buildDetailCard(
                              title: 'Diferença',
                              value: gasto['diferenca'] != null ? formatarValorDynamic(gasto['diferenca']) : 'Não disponível',
                              color: Colors.orange,
                    ...
                    orcamento['data_encerramento'] == null ? Column(
                      children: [
                        const SizedBox(height: 20),
                    // Botão Pagar (se o valor não foi pago)
                    if (gasto['valor'] == null) ...[
                      SizedBox(
                      ...
                      onPressed: () async {
                        bool? confirmDelete = await showDialog<bool>(
                          ...
                          builder: (context) => AlertDialog(
                            ...
                            content: const Text('Você tem certeza que deseja apagar este gasto fixo?'),
                            actions: [
                              TextButton(
                                onPressed: () {
                        ...
                        if (confirmDelete == true) {
                          await deleteGastoFixo(widget.gastoId);
                        }
                      },
                      style: ElevatedButton.styleFrom(
